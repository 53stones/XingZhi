...
public class AppAuthServiceImpl implements AppAuthService {
    ...
    public AppUserLoginVO login(AppUserLoginDTO dto) {
        ...
        if (PasswordUtil.isBcrypt(dbPassword))
        {
            passwordMatched = PasswordUtil.matches(inputPassword, dbPassword);
        }
        else
        {
            passwordMatched = inputPassword.equals(dbPassword);

            if (passwordMatched) {
                AppUser updateUser = new AppUser();
                updateUser.setId(user.getId());
                updateUser.setPassword(PasswordUtil.encode(inputPassword));
