...
public class AppAgentServiceImpl implements AppAgentService {
    private static final Logger log = LoggerFactory.getLogger(AppAgentServiceImpl.class);
    ...
    private String requestDify(Long userId, String message, MultipartFile image) {
        try {
            ...
            log.error("Dify request failed: {}", ex.getMessage(), ex);
            return null;
        } catch (Exception ex) {
            log.error("Dify request exception: {}", ex.getMessage(), ex);
            return null;
        }
    }
