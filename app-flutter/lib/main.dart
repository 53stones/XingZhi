...
import 'pages/OfflinesafetyPackagePage.dart';
...
class _XingZhiHomeState extends State<XingZhiHome> {
  ...
  Widget _buildPage() {
    return switch (_selectedTab) {
      ...
      AppTab.aid => MutualAidPage(
          key: const ValueKey('aid'),
          isDayMode: _isDayMode,
...
Positioned.fill(
  child: MediaQuery(
    ...
    child: Navigator(
      pages: [
        MaterialPage(
          key: const ValueKey('phone-root-page'),
          child: child,
        ),
      ],
...
class MutualAidPage extends StatefulWidget {
  const MutualAidPage({required this.isDayMode, required this.authToken, super.key});
...
class _MutualAidPageState extends State<MutualAidPage> {
  ...
  void initState() {
    super.initState();
    _loadNearbyHelpRequests();
  }

  @override
  void didUpdateWidget(covariant MutualAidPage oldWidget) {
  ...
  static const double _currentLatitude = 31.0621;
  static const double _currentLongitude = 102.8858;
  static const int _searchRadiusKm = 3;

  static double _distanceKm(double lat1, double lon1, double lat2, double lon2) {
    final radiansPerDegree = math.pi / 180;
    final dLat = (lat2 - lat1) * radiansPerDegree;
    final dLon = (lon2 - lon1) * radiansPerDegree;
  ...
  static String _formatDistance(double km) {
    if (km < 1) {
      return '${(km * 1000).round()} 米';
    }
    return '${km.toStringAsFixed(1)} 公里';
  }

  Future<void> _loadNearbyHelpRequests() async {
    ...
    try {
      final uri = Uri.parse('${BackendConfig.baseUrl}/app/help-events/nearby').replace(
        queryParameters: {
          'latitude': _currentLatitude.toString(),
          'longitude': _currentLongitude.toString(),
          'radius': _searchRadiusKm.toString(),
        },
      );
      debugPrint('Nearby help request URI: $uri');
      final headers = <String, String>{'Content-Type': 'application/json'};
      headers['Authorization'] = 'Bearer ${widget.authToken}';

      final response = await http.get(uri, headers: headers);
      if (response.statusCode != 200) {
        throw Exception('后端请求失败：${response.statusCode}');
      }

      final body = jsonDecode(utf8.decode(response.bodyBytes));
      if (body is! Map || body['code'] != 200) {
        throw Exception(body['msg']?.toString() ?? '返回格式异常');
      }

      final data = body['data'];
      if (data == null || data['records'] == null) {
        throw Exception('没有附近互助数据');
      }

      final records = data['records'] as List<dynamic>;
      final requests = records.map((item) {
        final map = item as Map<String, dynamic>;
        final title = map['helpType']?.toString() ?? '未知求助';
        final desc = map['helpModeName']?.toString() ?? '等待补充描述';
        ...
        final distance = (latitude != null && longitude != null)
            ? _formatDistance(_distanceKm(_currentLatitude, _currentLongitude, latitude, longitude))
            : '3公里内';
        final level = int.tryParse(map['emergencyLevel']?.toString() ?? '') ?? 1;
        final color = level >= 3 ? AppColors.orange : (level == 2 ? AppColors.blue : AppColors.green);
        return HelpRequest(
          title,
          desc,
          distance,
      ...
      children: [
        Row(
          children: [
            ...
            const Spacer(),
            SignalTag(isDayMode: widget.isDayMode),
          ],
        ),
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: widget.isDayMode ? Colors.white.withOpacity(0.64) : Colors.black.withOpacity(0.18),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                      color: widget.isDayMode ? const Color(0x66D9A966) : Colors.white.withOpacity(0.1),
                    ),
                  ),
                  ...
                  child: Text(
                    '描述你需要的帮助',
                    style: TextStyle(
                      color: widget.isDayMode ? const Color(0x991A120D) : AppColors.soft,
                      fontSize: 12,
                      fontWeight: FontWeight.w700,
                    ),
                  ),
              ...
              children: [
                ...
                if (_loading)
                  ...
                  Padding(
                    ...
                    child: Text(_error, style: const TextStyle(color: AppColors.orange, fontSize: 12, fontWeight: FontWeight.w700)),
                  )
                else if (_requests.isEmpty)
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 14),
                    child: Text('当前没有附近互助需求', style: TextStyle(color: AppColors.soft, fontSize: 12, fontWeight: FontWeight.w700)),
                  )
                else
                  for (final request in _requests)
                    HelpRequestRow(request: request, isDayMode: widget.isDayMode),
              ],
            ),
          ),
        ),
...
Future<List<EmergencyContact>> fetchEmergencyContacts({
  required int userId,
  required String token,
}) async {
  final response = await http.get(
    Uri.parse('${BackendConfig.baseUrl}/users/$userId/contact'),
    headers: {
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    throw Exception('查询紧急联系人失败：${response.statusCode}');
  }

  final body = jsonDecode(utf8.decode(response.bodyBytes));

  if (body is! Map || body['code'] != 200) {
    throw Exception(body['msg']?.toString() ?? '查询紧急联系人失败');
  }
...
class AuthSession {
  ...
  factory AuthSession.fromJson(Map<String, dynamic> json) {
    return AuthSession(
      id: int.tryParse(
      (json['id']??json['userId'])?.toString() ?? '',) ?? 0,
...
class EmergencyContact {
  ...
  factory EmergencyContact.fromJson(Map<String, dynamic> json) {
    return EmergencyContact(
      ...
      phone: json['phone']?.toString() ?? '',
      relation: json['relation']?.toString() ?? json['relationType']?.toString() ?? '',
    );
  }
}
              ...
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    isCollapsed: true,
                    contentPadding: EdgeInsets.only(top:0,bottom:3),
...
showDialog(
  ...
  builder: (ctx) => AlertDialog(
    insetPadding: const EdgeInsets.symmetric(horizontal: 18, vertical: 24),
        ...
        title: const Text('添加紧急联系人', style: TextStyle(fontWeight: FontWeight.w900)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
  ...
  void _deleteContact(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('确定要删除紧急联系人「${_contacts[index].name}」吗？'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('取消'),
  ...
  @override
  Widget build(BuildContext context) {
    final primaryText = widget.isDayMode ? const Color(0xFF1D140C) : Colors.white;
    final secondaryText = widget.isDayMode ? const Color(0xB21D140C) : AppColors.soft;
                ...
                child: Column(
                  ...
                  children: [
                    Icon(Icons.contact_emergency_rounded,
                        size: 64, color: AppColors.soft.withOpacity(0.4)),
                    ...
                    ConstrainedBox(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: AppColors.green.withOpacity(0.36),
                          ),
                        ),
                        child: Text(
                          contact.relation,
...
ProfileActionRow(
  ...
  onTap: () async {
  try {
    final contacts = await const BackendApiService().fetchEmergencyContacts(
      userId: userId,
      token: authToken,
    );
      ...
      MaterialPageRoute(
        builder: (_) => Scaffold(
          ...
          body: Stack(
            children: [
              Positioned.fill(
                child: MountainBackground(isDayMode: isDayMode),
              ),
              SafeArea(
                bottom: false,
                child: EmergencyContactsPage(
                  isDayMode: isDayMode,
                  contacts: contacts,
                  onContactsChanged: onEmergencyContactsChanged,
                ),
              ),
            ],
          ),
  ...
  onTap: () {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => Scaffold(
          ...
          body: Stack(
            children: [
              ...
              SafeArea(
                bottom: false,
                child: OfflineSafetyPackagePage(
                  isDayMode: isDayMode,
                ),
              ),
            ],
        ...
        return Padding(
          padding: EdgeInsets.fromLTRB(18, compact ? 18 : 18, 18, compact ? 6 : 14),
          child: compact
...
class ProfileActionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final primaryText = isDayMode ? const Color(0xFF1D140C) : Colors.white;
    final secondaryText = isDayMode ? const Color(0x991D140C) : AppColors.soft;
    final row = Padding(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                ...
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(color: primaryText,fontSize: expanded ? 15 : 13, fontWeight: FontWeight.w900),
              ),
            ),
